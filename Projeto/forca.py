...
p=turtle.Turtle() #p é o cursor da palavra
E=turtle.Turtle()# E é o cursor de aviso
l=[]
...
def caneta():
    ...
    #avisos e escolhas de menu:
    t.up()
    t.write( "Escolha uma categoria:",move=True,font=("",9))
    t.goto(0,-20)
    t.write("(1) Cores",move=True,font=("",9))
    t.goto(0,-40)
    t.write("(2)Frutas",move=True,font=("",9))
    t.goto(0,-60)
    t.write("(3)Objetos",move=True,font=("",9))
    t.goto(0,-80)
    t.write("(4)Veiculos",move=True,font=("",9))
    t.goto(0,-100)
    t.write("(5)Esportes",move=True,font=("",9))
    t.goto(0,-120)
    t.write("(6)Verbos",move=True,font=("",9))

    E.up()
    E.goto(0,20)
    E.setheading(0)
    E.pen(pencolor="black", pensize=3, speed=30)
...
def desenho_da_pessoa(n):
    '''função que desenha o corpo da pessoa enforcada membro a membro
...
def inicializa_letras_acertadas(palavra):
    """ Função que adiciona os espaços para cada letra da palavra escolhida """
...
def pede_chute():
    ...
    if chute not in l:
        l.append(chute)
        print('Já saíram as letras:', l, '\n')        
    else:
        print('A letra', chute, 'já saiu!')
        pede_chute()
...
def marca_chute_correto(chute, letras_acertadas, palavra_secreta):
    ...
    for letra in palavra_secreta:
        if (chute == letra):
            ...
            p.write( f"{letra} ",move=True,font=("",13)) #carrega as letras certas no desenho
        else:
...
def menu():
    # Nova funcionalidade no projeto, escolha da categoria das palavras
    opcao = (1,2,3,4,5,6)
    lista = ('Cores', 'Frutas', 'Objetos', 'Veiculos', 'Esportes', 'Verbos')
    menu = tuple(zip(opcao, lista))
    ...
    categoria = lista[int(escolha)-1].lower()
    print('A categoria escolhida é:', categoria, '\n')
    p.up()
    p.goto(40,40)
    p.write( f"Tema:{categoria}",move=True,font=("",9))
    p.goto(30,-200)
    p._update()
    t._update()
...
def carrega_palavra_secreta():
    ...
    # carrega a categoria ecolhida do menu
    categoria = menu()
    palavras = []
           
    with open(categoria, "r", encoding="utf-8") as arquivo:
...
def jogar():
    ...
    while (not acertou and not enforcou):
        ...
        if (chute in palavra_secreta):
            ...
            E.clear()
            E.home()
            E.write( f"Bom Chute, acertou uma",move=True,font=("",9))
            ...
            E.clear()
            E.home()
            E.write( f"Errou!!Faltam {7-erros} tentativas",move=True,font=("",9))
...
def play():
    # nova funcionalidade do jogo, opção de continuar jogando...
    E.write("Deseja jogar novamente? (Y | N):",move=True,font=("",9))
    jogardenovo=input("Deseja jogar novamente? (Y | N):").upper()
